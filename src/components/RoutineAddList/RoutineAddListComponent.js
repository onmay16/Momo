    ...
    const fetchData = async () => {
        try {
            ...
            for (const routine of routineCollection.documents) {
                ...
                const routineData = { id: tmp_id[(tmp_id.length - 1)], category: category.stringValue, name: routine_name.stringValue, duration: duration.integerValue, difficulty: difficulty.integerValue, emoji: emoji.stringValue };
        ...
        <SafeAreaView style={{position:'relative'}}>
           ...
           <View style={{height: props.isTutorial ? 110 : 90, alignItems:'flex-end', justifyContent:'flex-start', position:'absolute', right:0, bottom:0, width:'100%'}} pointerEvents="none">
                <View style={{flexDirection:'row', alignItems: 'center' , justifyContent:'flex-end' }}>
                    ...
                    <PretendardedText style={{fontWeight: '500', fontSize: 16, color: props.isTutorial ?  "#FFFFFF" : "#22222"}}>/28 </PretendardedText>
										<GreenStar/>
                </View>
                <PretendardedText style={{fontWeight: '500', fontSize: 16, color: props.isTutorial ? "#D9D9D9" : "#4C4C4C", textAlign:'right'}}>루틴을 마칠 시간까지</PretendardedText>
                <View style={{flexDirection:'row', justifyContent:'flex-end'}}>
                    <PretendardedText style={{fontWeight: '500', fontSize: 16, color: "#3CE3AC" }}>NN분 더 </PretendardedText>
                    <PretendardedText style={{fontWeight: '500', fontSize: 16, color: props.isTutorial ? "#D9D9D9" : "#4C4C4C" }}>여유있어요.</PretendardedText>
                </View>
                {
									props.isTutorial ?
